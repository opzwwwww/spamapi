...
import requests  # Import cho requests

app = Flask(__name__)

# Danh sách họ, tên... (giữ nguyên từ main.py)
...
def submit():
    try:
        ...
        delay = float(request.form['delay'])  # Chuyển sang float để sleep
        solanlap = int(request.form['solanlap'])  # Chuyển sang int

        # Thời gian hiện tại
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Log để debug
        print(f"\n--- DỮ LIỆU MỚI ({timestamp}) ---")
        print(f"Số Điện Thoại: {sdt}")
        print(f"Delay: {delay}")
        print(f"Số Lần Lặp: {solanlap}")

        # Ghi đè nội dung cũ bằng dữ liệu mới
        data_content = f"{sdt}|{delay}|{solanlap}\n"
        with open('data.txt', 'w', encoding='utf-8') as f:
            f.write(data_content)
        
        # Kiểm tra file đã ghi chưa (debug)
        if os.path.exists('data.txt'):
            with open('data.txt', 'r', encoding='utf-8') as f:
                read_content = f.read().strip()
            print(f"Đã ghi file data.txt: {read_content}")
        else:
            print("LỖI: Không tạo được file data.txt!")

        # CHẠY SPAM NGAY LẬP TỨC TRONG BACKGROUND THREAD (KHÔNG BLOCK WEB)
        spam_thread = threading.Thread(target=run_spam_once, args=(sdt, delay, solanlap))
        spam_thread.daemon = True  # Tự động dừng khi app dừng
        spam_thread.start()
        print(f"[THREAD] Đã khởi chạy spam thread cho {sdt}...")

        # Trở lại trang chủ với thông báo thành công
        return render_template('index.html', success=True)
    except Exception as e:
        print(f"LỖI submit: {e}")
        return render_template('index.html', success=False)

# Hàm spam CHỈ CHẠY MỘT LẦN (không loop), với đúng solanlap và delay
def run_spam_once(sdt, delay, solanlap):
    print(f"[SPAM] Bắt đầu spam cho {sdt}, {solanlap} lần, delay {delay}s")
    
    # Danh sách các hàm gửi OTP (gộp từ main.py, loại bỏ duplicate)
    otp_functions = [
        send_otp_via_sapo,
        ...
        # Thêm các hàm khác nếu thiếu (từ code gốc của bạn)
        send_otp_via_phuclong,  # Ví dụ
        send_otp_via_takomo,   # Ví dụ
    ]
    
    for lap in range(solanlap):
        print(f"[SPAM] Lần lặp thứ {lap + 1}/{solanlap}")
        
        # Chạy các hàm trong threads riêng (song song)
        threads = []
        for func in otp_functions:
            if callable(func):  # Kiểm tra hàm tồn tại
                thread = threading.Thread(target=func, args=(sdt,))
        ...
        if lap < solanlap - 1:  # Không delay sau lần cuối
            print(f"[SPAM] Hoàn thành lần {lap + 1}. Đợi {delay}s...")
            time.sleep(delay)
    
    print(f"[SPAM] HOÀN THÀNH {solanlap} LẦN CHO {sdt}. Chờ submit mới để spam tiếp.")
...
if __name__ == '__main__':
    print("Server Flask + Spam đang chạy tại http://0.0.0.0:5000")
